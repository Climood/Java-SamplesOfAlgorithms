...
public class binaryTree<typeOfValue> {    //Dublicates not allowed in this realization
    ...
    //------------------------------------------------------------------------------------
    binaryTree mergeTreeWithoutSorting(binaryTree leftTree,binaryTree rightTree) {  //Метод для использования в сжатии хаффмана (объединяет деревья в очереди)
        binaryTree<typeOfValue> newTree = new binaryTree<>();
        Node current = new Node();
        newTree.root = current;
        current.setKey(leftTree.root.getKey() + rightTree.root.getKey());
        current.leftChild = leftTree.root;
        leftTree.root.parent = current;
        current.rightChild = rightTree.root;
        rightTree.root.parent = current;
        current.setValue(Character.MAX_VALUE); //Ставим максимальное значение, использовать как маркер, что узел не имеет значения а лишь связующий?
        return newTree;
    }
